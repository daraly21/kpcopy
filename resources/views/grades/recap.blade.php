            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                ...
                <div class="mb-8 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        @if(auth()->user()->role_id == 1) {{-- Admin --}}
                            <div>
                                <form action="{{ route('grades.recap') }}" method="GET" class="space-y-2">
                                    ...
                                    <select id="class_id" name="class_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" onchange="this.form.submit()">
                        ...
                        @if($class_id)
                            <div>
                                <form action="{{ route('grades.recap') }}" method="GET" id="filterForm" class="space-y-2">
                                    <input type="hidden" name="class_id" value="{{ $class_id }}">
                                    <label for="subject_id" class="block text-sm font-medium text-gray-700">Mata Pelajaran:</label>
                                    <select id="subject_id" name="subject_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" onchange="document.getElementById('filterForm').submit()">
                            ...
                            <div>
                                <form action="{{ route('grades.recap') }}" method="GET" id="taskForm" class="space-y-2">
                                    <input type="hidden" name="class_id" value="{{ $class_id }}">
                                    @if($selectedSubject)
                                    ...
                                    <select id="task_name" name="task_name" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" onchange="document.getElementById('taskForm').submit()">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($grades as $index => $grade)
                                    <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}" id="grade-row-{{ $grade->id }}">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $index + 1 }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ $grade->student_name }}</div>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="score-display px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full 
                                                {{ $grade->score >= 90 ? 'bg-green-100 text-green-800' : 
                                                   ($grade->score >= 75 ? 'bg-blue-100 text-blue-800' : 
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2 items-center">
                                                <!-- Edit Button -->
                                                <button class="edit-btn p-1 text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 rounded-md transition" 
                                                        data-grade-id="{{ $grade->id }}"
                                                        title="Edit Nilai">
                                                ...
                                                <button class="save-btn hidden p-1 text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100 rounded-md transition" 
                                                        data-grade-id="{{ $grade->id }}"
                                                ...
                                                <button class="cancel-btn hidden p-1 text-gray-600 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 rounded-md transition" 
                                                        data-grade-id="{{ $grade->id }}"
                                                        title="Batal Edit">
...
function saveGrade(gradeId) {
    ...
    .then(data => {
        if (data.success) {
            ...
            gradeDisplay.className = 'score-display px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full ';
            if (newScore >= 90) {
...
function showAlert(type, message) {
    ...
    alertDiv.className = `mb-4 p-4 rounded-lg border-l-4 ${type === 'error' ? 'bg-red-50 border-red-500 text-red-700' : 'bg-green-50 border-green-500 text-green-700'}`;
    alertDiv.innerHTML = `
